...
ostream& operator<<(ostream &os, const ReadCatalog& other){
    ...
    outputString += string("Searched authors: ") + authorsToFind[0].substr(authorsToFind[0].rfind(" ")+1, authorsToFind[0].length()-1);
    for(int i=1;i<5;i++){
        outputString += ", " + authorsToFind[i].substr(authorsToFind[i].rfind(" ")+1, authorsToFind[i].length()-1);
    }
    outputString += string("\n");
    ...
    while(other.isNextAuthor()){
        //Gets the next author in the file
        lineAuthor = other.getNextAuthor();
        ...
        for(int i=0;i<5;i++){
            ...
            if(lineAuthor == other.removePunct(authorToCompare)){
    ...
    outputString += string("Author Occurrence (*not representative)") + string("\n");
    ...
    for(int i=0;i<5;i++){
        ...
        appropiateSpaces = 15-authorToCompare.length();
        outputString += authorToCompare + string(appropiateSpaces, ' ') + string(barsToDraw, '=') + string(" ");
        outputString += to_string(authorOccurrences[i]) + string(" ( ") + to_string((authorOccurrences[i]*100 / maxAuthorOccurrences)) + string("%)") + string("\n");

    }
    outputWriter << outputString;
    outputWriter.close();
    authorReader.close();
    os << outputString;
    return os;
