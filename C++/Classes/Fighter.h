...
class Fighter{
    ...
    public:
        ...
        Fighter(int playerNum){
            ...
            count++;
        }; //This is the consutrctor - runs when the instance of the class is first created
        ...
        //We use const here as we aren't really changing anything and it simultaneously allows us to view its private variables
        bool operator>(const Fighter &other) const;

        //Type conversion
        operator string() const;

        //Friend keyword  stands for friend function
            //This function has access to any private member functions which can be used outside of the class
        friend ostream& operator<<(ostream &outputStream, const Fighter &other);
